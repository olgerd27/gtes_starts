...
DBTEditor::DBTEditor(const dbi::DBTInfo *dbtInfo, QWidget *parent)
    ...
    , m_proxyDcrMdl1(new ProxyDecorModel(nullptr))
    , m_proxyFltrMdl2(new ProxyFilterModel(nullptr))
...
{
    m_proxyDcrMdl1->setSqlTable(m_DBTInfo->m_nameInDB);
    m_proxyFltrMdl2->setSourceModel(m_proxyDcrMdl1);
    m_proxyFltrMdl2->setFilterCaseSensitivity(Qt::CaseInsensitive);
    m_proxyFltrMdl2->setFilterKeyColumn(-1);
}
...
//    m_mapper->setModel(m_proxyDcrMdl1);
    m_mapper->setModel(m_proxyFltrMdl2);
}

void DBTEditor::setSelectUI()
{
    auto view = m_ui->m_tableContents; // get the table view
    m_ui->m_tableContents->setModel(m_proxyFltrMdl2);
//    m_ui->m_tableContents->setModel(m_proxyDcrMdl1);
...
//            m_proxyDcrMdl1, SLOT(slotChooseRow(QItemSelection,QItemSelection)));
    connect(view->selectionModel(), SIGNAL(selectionChanged(QItemSelection,QItemSelection)),
            m_proxyFltrMdl2, SLOT(slotChooseRow(QItemSelection,QItemSelection)));
}
...
void DBTEditor::setFilter()
{
    ...
    SelectionAllower_IC *sa = new SelectionAllower_IC(m_ui->m_leFilter, m_ui->m_tableContents, m_proxyFltrMdl2);
    m_proxyFltrMdl2->setSelectionAllower(sa);
    connect(m_proxyFltrMdl2, SIGNAL(sigSelectionEnded()), sa, SLOT(slotSelectionEnded()));
    connect(m_ui->m_leFilter, SIGNAL(textChanged(QString)), m_proxyFltrMdl2, SLOT(setFilterFixedString(QString)));
}
...
{
    connect(m_ui->m_pbAdd, SIGNAL(clicked()), m_proxyDcrMdl1, SLOT(slotAddRow()));
    connect(m_ui->m_pbDelete, &QPushButton::clicked, [this]()
    {
        m_proxyDcrMdl1->slotDeleteRow( m_mapper->currentIndex() );
    } );
    connect(m_ui->m_pbSave, &QPushButton::clicked, [this]()
    {
        m_proxyDcrMdl1->slotSaveDataToDB( m_mapper->currentIndex() );
    } );
    connect(m_ui->m_pbRefresh, &QPushButton::clicked, [this]()
    {
        m_proxyDcrMdl1->slotRefreshModel( m_mapper->currentIndex() );
    } );
}

void DBTEditor::setDataNavigation()
{
    connect(m_proxyDcrMdl1, SIGNAL(sigChangeCurrentRow(int)), m_mapper, SLOT(setCurrentIndex(int)));
...
{
    m_initSelectRow = -1;
    ASSERT_DBG( m_proxyDcrMdl1->customSourceModel()->getIdRow(idPrim, m_initSelectRow),
...
{
    return m_proxyDcrMdl1->selectedId();
}

void DBTEditor::accept()
{
    if (m_proxyDcrMdl1->customSourceModel()->isDirty()) {
        /*
        ...
        if (autoSave) m_proxyDcrMdl1->slotSaveDataToDB( m_mapper->currentIndex() );
        else askSaving();
    }
    QDialog::accept();
}
...
void DBTEditor::askSaving()
{
    ...
    case QMessageBox::Save:
        m_proxyDcrMdl1->slotSaveDataToDB( m_mapper->currentIndex() ); // save data to the DB
        break;
    case QMessageBox::Discard:
        // Delete all changes and return initial Id value
        m_proxyDcrMdl1->clearDirtyChanges(); // this allow select initial row (below) if it is marked "deleted" and not saved in the DB
...
{
    const QModelIndex &currIndex = m_proxyDcrMdl1->index( m_mapper->currentIndex(), fieldNo + ProxyDecorModel::COUNT_ADDED_COLUMNS );
    const QVariant &forId = m_proxyDcrMdl1->data(currIndex, Qt::UserRole);
    DBTEditor childEditor(dbtInfo, this);
    connect(childEditor.m_proxyDcrMdl1->customSourceModel(), SIGNAL(sigSavedInDB()),
            m_proxyDcrMdl1->customSourceModel(), SLOT(slotRefreshTheModel())); // save changes in the child model -> refresh the parent (current) model
    ...
    if ( childEditor.exec() == QDialog::Accepted ) {
        ...
        ASSERT_DBG( m_proxyDcrMdl1->setData( currIndex, childEditor.selectedId(), Qt::EditRole ),
...
{
    const QModelIndex &currIndex = m_proxyDcrMdl1->index( m_mapper->currentIndex(), m_mapper->mappedSection(w) );
    m_proxyDcrMdl1->setData(currIndex, data, Qt::EditRole);
}
